                        ...
                        progressTimerRunnable = object : Runnable {
                            override fun run() {
                                ...
                                if (countdownValue == 0 && valueOfProgress < 100) {
                                    ...
                                    Log.e(
                                        TAG,
                                        "Countdown reached 0. Cleaning up resources and refreshing time. val = $valueOfProgress"
                                    )
                                    ...
                                    Log.d(
                                        TAG,
                                        "Progress else. elseelseelseelseelseelseelseelseelseelseelseelse else."
                                    )
...
//    }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                printer.print(adapter, path, temporaryFileName, object : PdfPrinter.Callback {
                    override fun onSuccess(filePath: String) {
                        Log.d(TAG, "PDF generation succeeded. File path: $filePath")
                        // Add shadows to the generated PDF
                        addShadowsToPdf(filePath, callback)
                    }
    ...
    private fun addShadowsToPdf(pdfFilePath: String, callback: Callback) {
        try {
            val shadowedPdfPath = pdfFilePath.replace(".pdf", "_shadowed.pdf")

            if (pdfFilePath == shadowedPdfPath) {
                throw IllegalArgumentException("Input and output PDF paths must be different.")
            }

            val pdfReader = PdfReader(pdfFilePath)
            val pdfWriter = PdfWriter(shadowedPdfPath)
            ...
            for (i in 1..pdfDocument.numberOfPages) {
                ...
                val extGState = PdfExtGState().apply { fillOpacity = 0.3f }
                canvas.setExtGState(extGState)
                canvas.setFillColor(ColorConstants.GRAY)

                val shadowThickness = 5f

                // Top shadow
                canvas.rectangle(
                    pageSize.left.toDouble(),
                    pageSize.top.toDouble() - shadowThickness,
                    pageSize.width.toDouble(),
                    shadowThickness.toDouble()
                )
                // Bottom shadow
                canvas.rectangle(
                    pageSize.left.toDouble(),
                ...
                // Right shadow
                canvas.rectangle(
                    pageSize.right.toDouble() - shadowThickness,
                    pageSize.bottom.toDouble(),
                    shadowThickness.toDouble(),
                    pageSize.height.toDouble()
                )
            ...
            // Cleanup the original PDF file if desired (optional)
            File(pdfFilePath).delete()

            callback.onSuccess(shadowedPdfPath)
        } catch (e: Exception) {
            Log.e("PDFShadowError", "Failed to add shadows to PDF: ${e.message}", e)
            callback.onFailure()
        } finally {
            cleanupWebView()
        }
